...
List<ToolResult> calculateTool(ToolDefinition tool, Map<String, double> v) {
  switch (tool.kind) {
    case ToolKind.quadratic:
      ...
      final x2 = (-b - root) / (2 * a);
      return [
        ToolResult('解 (x1, x2)', '${formatNumber(x1)}  ${formatNumber(x2)}', '', primary: true),
    ...
    case ToolKind.linearSystem:
      ...
      return [
        ToolResult('解 (x, y)', '${formatNumber(x)}  ${formatNumber(y)}', '', primary: true),
    ...
    case ToolKind.exponentialLog:
      ...
      return [
        ToolResult('x^y', formatNumber(math.pow(x, y).toDouble()), '', primary: true),
        ToolResult('ln(x)', x <= 0 ? '无效' : formatNumber(math.log(x)), ''),
        ToolResult('log10(x)', x <= 0 ? '无效' : formatNumber(math.log(x) / math.ln10), ''),
    ...
    case ToolKind.linearEquation:
      ...
      final b = v['b'] ?? 0;
      return [
        ToolResult('解 x', a == 0 ? '无唯一解' : formatNumber(-b / a), '', primary: true),
    ...
    case ToolKind.proportion:
      ...
      return [
        ToolResult('x', a == 0 ? '无效' : formatNumber(b * c / a), '', primary: true),
    ...
    case ToolKind.combination:
      ...
      return [
        ToolResult('组合 C(n,k)', valid ? formatNumber(c) : '无效', '', primary: true),
    ...
    case ToolKind.statistics:
      ...
      final variance = values.map((item) => math.pow(item - mean, 2)).reduce((a, b) => a + b) / (values.length - 1);
      return [
    ...
    case ToolKind.complex:
      final a = v['a'] ?? 0, b = v['b'] ?? 0, c = v['c'] ?? 0, d = v['d'] ?? 0;
      return [
        ToolResult('z1 + z2', '${formatNumber(a + c)} ${_signedImag(b + d)}i', '', primary: true),
        ToolResult('z1 - z2', '${formatNumber(a - c)} ${_signedImag(b - d)}i', ''),
        ToolResult('z1 × z2', '${formatNumber(a * c - b * d)} ${_signedImag(a * d + b * c)}i', ''),
        ToolResult('|z1|', formatNumber(math.sqrt(a * a + b * b)), ''),
      ];
    case ToolKind.vector:
      final x1 = v['x1'] ?? 0, y1 = v['y1'] ?? 0, x2 = v['x2'] ?? 0, y2 = v['y2'] ?? 0;
      final dot = x1 * x2 + y1 * y2;
      final cross = x1 * y2 - y1 * x2;
      final len1 = math.sqrt(x1 * x1 + y1 * y1), len2 = math.sqrt(x2 * x2 + y2 * y2);
      final cos = len1 == 0 || len2 == 0 ? 0 : (dot / (len1 * len2)).clamp(-1.0, 1.0);
      return [
        ...
        ToolResult('|A| / |B|', '${formatNumber(len1)} / ${formatNumber(len2)}', ''),
      ];
    case ToolKind.triangle:
      ...
      final angleA = math.acos(((b * b + c * c - a * a) / (2 * b * c)).clamp(-1.0, 1.0)) * 180 / math.pi;
      final angleB = math.acos(((a * a + c * c - b * b) / (2 * a * c)).clamp(-1.0, 1.0)) * 180 / math.pi;
      return [
        ToolResult('面积', formatNumber(area), '', primary: true),
        ToolResult('周长', formatNumber(a + b + c), ''),
        ToolResult('角 A/B/C', '${formatNumber(angleA)} / ${formatNumber(angleB)} / ${formatNumber(180 - angleA - angleB)}', 'deg'),
      ];
    ...
    case ToolKind.scaleRatio:
      ...
      return [
        ...
        ToolResult('面积比例', from == 0 ? '0' : formatNumber(math.pow(to / from, 2).toDouble()), 'x'),
      ];
    ...
    case ToolKind.voltageDivider:
      ...
      return [
        ToolResult('输出电压 Vout', formatNumber(vout, precision: 4), 'V', primary: true),
        ToolResult('分压电流', formatNumber(current * 1000), 'mA'),
        ToolResult('总功耗', formatNumber(vin * current * 1000), 'mW'),
        ToolResult('误差范围', '${formatNumber(vout * (1 - tol / 100))}~${formatNumber(vout * (1 + tol / 100))}', 'V'),
      ];
    case ToolKind.rcFilter:
      final r = (v['r'] ?? 0) * 1000;
      final c = (v['c'] ?? 0) * 1e-9;
      if (r == 0 || c == 0) return const [ToolResult('截止频率', '0', 'Hz', primary: true)];
    ...
    case ToolKind.resistorNetwork:
      ...
      return [
        ToolResult('串联等效', formatNumber(r1 + r2), 'Ω', primary: true),
    ...
    case ToolKind.capacitorNetwork:
      ...
      return [
        ToolResult('并联等效', formatNumber(c1 + c2), 'nF', primary: true),
    ...
    case ToolKind.inductorNetwork:
      ...
      return [
        ToolResult('串联等效', formatNumber(l1 + l2), 'mH', primary: true),
        ToolResult('并联等效', formatNumber(parallel), 'mH'),
      ];
    case ToolKind.ledResistor:
      final vin = v['vin'] ?? 0, vf = v['vf'] ?? 0, currentMa = v['current'] ?? 0;
      ...
      return [
        ...
        ToolResult('功耗', formatNumber(current * current * resistance * 1000), 'mW'),
        ToolResult('推荐功率', current * current * resistance > 0.125 ? '≥ 1/4W' : '≥ 1/8W', ''),
      ];
    ...
    case ToolKind.dcdcFeedback:
      final vref = v['vref'] ?? 0, rtop = v['rtop'] ?? 0, rbottom = v['rbottom'] ?? 0;
    ...
    case ToolKind.ldoPower:
      final vin = v['vin'] ?? 0, vout = v['vout'] ?? 0, current = (v['current'] ?? 0) / 1000;
    ...
    case ToolKind.capacitorCharge:
      final vin = v['vin'] ?? 0, r = (v['r'] ?? 0) * 1000, c = (v['c'] ?? 0) * 1e-6, time = v['time'] ?? 0;
      final tau = r * c;
    ...
    case ToolKind.batteryLife:
      final capacity = v['capacity'] ?? 0, current = v['current'] ?? 0, eta = (v['efficiency'] ?? 100) / 100;
    ...
    case ToolKind.pcbCurrent:
      ...
      final rise = v['rise'] ?? 0;
      final current = area <= 0 || rise <= 0 ? 0 : 0.048 * math.pow(rise, 0.44) * math.pow(area, 0.725);
      return [
    ...
    case ToolKind.wireVoltageDrop:
      final current = v['current'] ?? 0, length = v['length'] ?? 0, area = v['area'] ?? 0, voltage = v['voltage'] ?? 0;
      ...
      return [
        ...
        ToolResult('压降比例', voltage == 0 ? '0' : formatNumber(drop / voltage * 100), '%'),
        ToolResult('线阻', formatNumber(resistance), 'Ω'),
        ToolResult('线损', formatNumber(drop * current), 'W'),
      ];
    case ToolKind.timer555:
      final ra = (v['ra'] ?? 0) * 1000, rb = (v['rb'] ?? 0) * 1000, c = (v['c'] ?? 0) * 1e-6;
      final high = 0.693 * (ra + rb) * c;
      final low = 0.693 * rb * c;
      final period = high + low;
      return [
        ToolResult('频率', period == 0 ? '0' : formatNumber(1 / period), 'Hz', primary: true),
        ToolResult('高电平时间', formatNumber(high * 1000), 'ms'),
        ToolResult('低电平时间', formatNumber(low * 1000), 'ms'),
        ToolResult('占空比', period == 0 ? '0' : formatNumber(high / period * 100), '%'),
      ];
    case ToolKind.thermalRise:
      final power = v['power'] ?? 0, theta = v['theta'] ?? 0, ambient = v['ambient'] ?? 0;
    ...
    case ToolKind.gearRatio:
      final z1 = v['z1'] ?? 0, z2 = v['z2'] ?? 0, rpm = v['rpm'] ?? 0, torque = v['torque'] ?? 0;
    ...
    case ToolKind.inclinedPlane:
      final mass = v['mass'] ?? 0, angle = (v['angle'] ?? 0) * math.pi / 180, mu = v['mu'] ?? 0;
    ...
    case ToolKind.beamBending:
      ...
      final deflection = elastic == 0 || inertia == 0 ? 0 : load * math.pow(length, 3) / (48 * elastic * inertia);
      return [
        ToolResult('最大挠度', formatNumber(deflection * 1000), 'mm', primary: true),
        ToolResult('最大弯矩', formatNumber(load * length / 4), 'N·m'),
        ToolResult('刚度 F/δ', deflection == 0 ? '0' : formatNumber(load / deflection), 'N/m'),
      ];
    case ToolKind.stressStrain:
      final force = v['force'] ?? 0, area = (v['area'] ?? 0) * 1e-6, elastic = (v['elastic'] ?? 0) * 1e9;
    ...
    case ToolKind.sectionArea:
      final diameter = v['diameter'] ?? 0, outer = v['outer'] ?? 0, inner = v['inner'] ?? 0;
      final width = v['width'] ?? 0, height = v['height'] ?? 0;
      return [
        ToolResult('圆截面积', formatNumber(math.pi * diameter * diameter / 4), 'mm²', primary: true),
        ToolResult('管截面积', formatNumber(math.pi * (outer * outer - inner * inner) / 4), 'mm²'),
    ...
    case ToolKind.safetyFactor:
      ...
      return [
        ...
        ToolResult('判断', factor >= 2 ? '较安全' : factor >= 1 ? '临界' : '失效风险', ''),
      ];
    ...
    case ToolKind.materialWeight:
      final length = v['length'] ?? 0, width = v['width'] ?? 0, thickness = v['thickness'] ?? 0, density = v['density'] ?? 0;
    ...
    case ToolKind.loan:
      ...
      final payment = monthlyRate == 0 ? principal / months : principal * monthlyRate * factor / (factor - 1);
      return [
    ...
    case ToolKind.annuity:
      final payment = v['payment'] ?? 0, rate = (v['rate'] ?? 0) / 100, years = v['years'] ?? 0, perYear = (v['perYear'] ?? 1).round();
    ...
    case ToolKind.installment:
      final price = v['price'] ?? 0, fee = (v['fee'] ?? 0) / 100, months = (v['months'] ?? 0).round();
      final total = price * (1 + fee);
      return [
        ToolResult('每期付款', months == 0 ? '0' : formatNumber(total / months), '元', primary: true),
        ToolResult('手续费', formatNumber(price * fee), '元'),
        ToolResult('总支付', formatNumber(total), '元'),
      ];
    case ToolKind.breakEven:
      final fixed = v['fixed'] ?? 0, price = v['price'] ?? 0, variable = v['variable'] ?? 0;
      ...
      return [
        ...
        ToolResult('边际率', price == 0 ? '0' : formatNumber(margin / price * 100), '%'),
      ];
    case ToolKind.electricityCost:
      final power = (v['power'] ?? 0) / 1000, hours = v['hours'] ?? 0, days = v['days'] ?? 0, price = v['price'] ?? 0;
      ...
      return [
        ...
        ToolResult('日均费用', days == 0 ? '0' : formatNumber(energy * price / days), '元/天'),
      ];
    case ToolKind.compound:
      ...
      return [
        ...
        ToolResult('收益率', formatNumber(principal == 0 ? 0 : (fv - principal) / principal * 100), '%'),
      ];
    case ToolKind.profitMargin:
      ...
      return [
        ...
        ToolResult('毛利率', formatNumber(price == 0 ? 0 : profit / price * 100), '%'),
        ToolResult('加价率', formatNumber(cost == 0 ? 0 : profit / cost * 100), '%'),
      ];
    case ToolKind.roi:
      final gain = v['gain'] ?? 0, cost = v['cost'] ?? 0;
      return [
        ToolResult('ROI', formatNumber(cost == 0 ? 0 : (gain - cost) / cost * 100), '%', primary: true),
    ...
    case ToolKind.tax:
      ...
      return [
        ...
        ToolResult('不含税反推', formatNumber(rate == -1 ? 0 : net / (1 + rate)), '元'),
      ];
    case ToolKind.inflation:
      final amount = v['amount'] ?? 0, rate = (v['rate'] ?? 0) / 100, years = v['years'] ?? 0;
      final future = amount * math.pow(1 + rate, years);
      return [
        ToolResult('未来等值', formatNumber(future.toDouble()), '元', primary: true),
        ToolResult('购买力折现', formatNumber(amount / math.pow(1 + rate, years)), '元'),
      ];
    case ToolKind.npv:
      final initial = v['initial'] ?? 0, rate = (v['rate'] ?? 0) / 100;
      final cf1 = v['cf1'] ?? 0, cf2 = v['cf2'] ?? 0, cf3 = v['cf3'] ?? 0;
      final npv = cf1 / (1 + rate) + cf2 / math.pow(1 + rate, 2) + cf3 / math.pow(1 + rate, 3) - initial;
      return [
        ToolResult('NPV', formatNumber(npv), '元', primary: true),
        ToolResult('总现金流', formatNumber(cf1 + cf2 + cf3), '元'),
      ];
    case ToolKind.length:
      return _unitResults(v['value'] ?? 0, {'m': 1, 'cm': 100, 'mm': 1000, 'in': 39.3701, 'ft': 3.28084});
    case ToolKind.area:
      return _unitResults(v['value'] ?? 0, {'m²': 1, 'cm²': 10000, 'mm²': 1000000, 'ft²': 10.7639});
    case ToolKind.volume:
      return _unitResults(v['value'] ?? 0, {'m³': 1, 'L': 1000, 'mL': 1000000, 'ft³': 35.3147});
    case ToolKind.mass:
      return _unitResults(v['value'] ?? 0, {'kg': 1, 'g': 1000, 'mg': 1000000, 'lb': 2.20462});
    case ToolKind.pressure:
      return _unitResults(v['value'] ?? 0, {'Pa': 1, 'kPa': 0.001, 'MPa': 0.000001, 'bar': 0.00001, 'psi': 0.000145038});
    case ToolKind.speed:
      return _unitResults(v['value'] ?? 0, {'m/s': 1, 'km/h': 3.6, 'mph': 2.23694, 'ft/s': 3.28084});
    ...
    case ToolKind.voltage:
      return _unitResults(v['value'] ?? 0, {'V': 1, 'mV': 1000, 'kV': 0.001, 'μV': 1000000, 'MV': 0.000001});
    case ToolKind.frequency:
      return _unitResults(v['value'] ?? 0, {'Hz': 1, 'kHz': 0.001, 'MHz': 0.000001, 'GHz': 0.000000001});
    case ToolKind.dataSize:
      return _unitResults(v['value'] ?? 0, {'B': 1, 'KB': 1 / 1024, 'MB': 1 / 1048576, 'GB': 1 / 1073741824});
    case ToolKind.timeUnit:
      return _unitResults(v['value'] ?? 0, {'s': 1, 'min': 1 / 60, 'h': 1 / 3600, 'day': 1 / 86400});
    case ToolKind.accelerationUnit:
      return _unitResults(v['value'] ?? 0, {'m/s²': 1, 'g': 1 / 9.80665, 'ft/s²': 3.28084});
    case ToolKind.forceUnit:
      return _unitResults(v['value'] ?? 0, {'N': 1, 'kN': 0.001, 'kgf': 1 / 9.80665, 'lbf': 0.224809});
    case ToolKind.powerUnit:
      ...
      return [
        ...
        ToolResult('dBm', watts <= 0 ? '无效' : formatNumber(10 * math.log(watts * 1000) / math.ln10), 'dBm'),
      ];
    case ToolKind.energyUnit:
      return _unitResults(v['value'] ?? 0, {'J': 1, 'kJ': 0.001, 'Wh': 1 / 3600, 'kWh': 1 / 3600000});
    case ToolKind.angleUnit:
      return _unitResults(v['value'] ?? 0, {'deg': 1, 'rad': math.pi / 180, 'turn': 1 / 360});
    case ToolKind.currentUnit:
      return _unitResults(v['value'] ?? 0, {'A': 1, 'mA': 1000, 'μA': 1000000});
    case ToolKind.resistanceUnit:
      return _unitResults(v['value'] ?? 0, {'Ω': 1, 'kΩ': 0.001, 'MΩ': 0.000001});
    case ToolKind.capacitanceUnit:
      return _unitResults(v['value'] ?? 0, {'F': 1, 'μF': 1000000, 'nF': 1000000000, 'pF': 1000000000000});
    case ToolKind.inductanceUnit:
      return _unitResults(v['value'] ?? 0, {'H': 1, 'mH': 1000, 'μH': 1000000});
    case ToolKind.torqueUnit:
      return _unitResults(v['value'] ?? 0, {'N·m': 1, 'kgf·m': 1 / 9.80665, 'lbf·ft': 0.737562});
    case ToolKind.flowUnit:
      return _unitResults(v['value'] ?? 0, {'L/min': 1, 'm³/h': 0.06, 'GPM': 0.264172});
    ...
    case ToolKind.kineticEnergy:
      final mass = v['mass'] ?? 0, speed = v['speed'] ?? 0, height = v['height'] ?? 0;
    ...
    case ToolKind.concentration:
      final mass = v['mass'] ?? 0, volume = v['volume'] ?? 0, molarMass = v['molarMass'] ?? 0;
    ...
    case ToolKind.idealGas:
      ...
      final pressure = volumeL == 0 ? 0 : n * 8.314462618 * temp / (volumeL / 1000);
      return [
    ...
    case ToolKind.wavelength:
      ...
      return [
        ...
        ToolResult('周期', frequency == 0 ? '0' : formatNumber(1 / frequency), 's'),
        ToolResult('频率', formatNumber(frequency), 'Hz'),
      ];
    case ToolKind.halfLife:
      final initial = v['initial'] ?? 0, half = v['half'] ?? 0, time = v['time'] ?? 0;
      final remaining = half <= 0 ? 0 : initial * math.pow(0.5, time / half);
      return [
        ToolResult('剩余量', formatNumber(remaining.toDouble()), '', primary: true),
        ToolResult('剩余比例', initial == 0 ? '0' : formatNumber(remaining / initial * 100), '%'),
    ...
    case ToolKind.bmi:
      ...
      final label = bmi < 18.5 ? '偏瘦' : bmi < 24 ? '正常' : bmi < 28 ? '超重' : '肥胖';
      return [
    ...
    case ToolKind.fuelEconomy:
      final distance = v['distance'] ?? 0, fuel = v['fuel'] ?? 0, price = v['price'] ?? 0;
      ...
      return [
        ...
        ToolResult('每公里成本', distance == 0 ? '0' : formatNumber(cost / distance), '元/km'),
      ];
